...
# Smart Caching Layers to Protect Free Limits
ODDS_CACHE = {}
...
def run_full_monte_carlo(home_xG=1.65, away_xG=1.20):
   ...
    
    # 15-Factor Physics & Micro-Climate Multipliers
   ...
    
    ...
    for _ in range(simulations):
        # Poisson distribution simulation based on adjusted xG
...
def generate_ai_insight(match_name, sim_data):
   ...
    
    prompt = (
        ...
        f"Stadium Weather -> Temp: {sim_data['weather']['temp']}°C, Wind: {sim_data['weather']['wind']} km/h. "
        "Provide a sharp, professional 3-sentence betting insight emphasizing expected value (+EV)."
    )
    try:
        ...
        if odds_data and len(odds_data) > 0:
           ...
            
            report = (
                ...
                f"🌡️ Climate: {sim['weather']['temp']}°C | Wind: {sim['weather']['wind']} km/h\n\n"
                f"🧠 *Gemini AI Insight:*\n{ai_text}\n\n"
            ...
            await query.edit_message_text(text="⚠️ Live odds currently unavailable or API limit reached. Using internal matrix.")
