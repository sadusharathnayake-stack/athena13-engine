import os, math, requests, sqlite3, logging
import numpy as np
...
HEADERS = {'x-rapidapi-host': "v3.football.api-sports.io", 'x-rapidapi-key': API_FOOTBALL_KEY if API_FOOTBALL_KEY else ""}

def dixon_coles_tau(x, y, lambda_, mu, rho=-0.13):
    if x == 0 and y == 0: return 1.0 - (lambda_ * mu * rho)
    if x == 1 and y == 0: return 1.0 + (mu * rho)
    if x == 0 and y == 1: return 1.0 + (lambda_ * rho)
...
def fetch_team_xg_stats(team_name):
    try:
        ...
        if stats_res.get('response'):
            for idx, match in enumerate(stats_res['response']):
        ...
        return {"name": team_official_name, "avg_scored": max(weighted_s * 0.95, 0.35), "avg_conceded": max(weighted_c * 0.95, 0.35)}
...
def main():
    if not TELEGRAM_BOT_TOKEN:
        print("CRITICAL: TELEGRAM_BOT_TOKEN missing in Railway Variables!")
        return
